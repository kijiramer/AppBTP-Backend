...
export default function Constatation({ route, navigation }) {
    ...
    const loadConstatations = async () => {
        try {
            ...
            const response = await axios.get(`http://89.84.57.170:8081/constatations?city=${city}&building=${building}&task=${task}&selectedDate=${selectedDate.toISOString()}`, {
                headers: {
    ...
    const addConstatation = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://89.84.57.170:8081/constatations', constatationData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
